...
sendfrendreq()
{
	msg:="|<msg>*118$16.zzy00Q03Y0a00M91U0600M01k0Dzzs"
	MouseMove	3182,1565
	Sleep 200
 if (ok:=FindText(3182-50, 1567-50, 3182+50, 1567+50, 0, 0, msg))
	{
		xx:=ok.1.x, yy:=ok.1.y
		MouseClick,left,xx,yy,,0,,
		sleep 450
		MouseClick,left,xx,yy
	}
	loop 10 ;% codes.MaxIndex()
	{
		MouseClick,left,xx,yy
		sleep 450
		MouseClick,left,xx,yy
		sleep 250
        MouseClick,left,-127,29,,40,, R
		sleep 250
 		a1 := codes[A_Index]
		SendInput, %a1%
        sendreq()
		if not mod(A_Index,5)
        {
            ;OutputDebug accept req `n
			acceptreq()
        }
        sleep 1050
	}
...
sendreq()
{
    ...
    loop,11
    {
    Text:="|<cancel_button_window>*112$30.zzzzzlryATVaE87ZW0dbbe0s7Z20dbVME81zzzzzU"
    OutputDebug cancel_button_window `n
     if (ok:=FindText(1047-150, 1264-150, 1047+150, 1264+150, 0, 0, Text))
     {
    ...
    Sleep 500
    Click,1262,1087
    Text:="|<ico_to_open_inv_window>*131$14.zzz3zUTs7y1zkzzxw0S01U0M0Tzrzzs"
OutputDebug ico_to_open_inv_window `n
 while !(ok:=FindText(1137-15, 1116-15, 1137+15, 1116+15, 0, 0, Text))
 {
...
Text:="|<inv_input>*58$33.000007zzzzzzzzzzzzzzzzzzzzzzzzzzzNaPNaQnAwnAaNbaNYnAwnANaPNaTzzzzzzzzzzzzzzzzzzzzzs00000U"
OutputDebug inv_input `n
 while !(ok:=FindText(876-50, 1208-50, 876+50, 1208+50, 0, 0, Text))
 {
 }
 xc:=ok.1.x, yc:=ok.1.y, Comment:=ok.1.id
   Click, %xc%, %yc%

OutputDebug im input `n
;.........
a1 := codes[i]
OutputDebug %a1% `n
SendInput, %a1%
;........
    ...
    Text:="|<q_dp>*106$7.zssAqvtswyzTzrvzs"
    OutputDebug q_dp `n
     while !(ok:=FindText(865-50, 1220-50, 865+50, 1220+50, 0, 0, Text))
     {}
        X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
       MouseClick, left,%X%,%Y%
    Text:="|<frnd+req+ico>*130$14.zzz3zUTs7y1zkzzxw0S01U0M0Tzrzzs"
    frnd_req_already_sent:="|<frnd_req_already_sent>*134$8.3kQ71kzz60U068"
    already_frnd:="|<already_frnd>*132$12.k10000004Q4QA8C8400000U0U"
    OutputDebug frnd+req+ico `n
    dont_clcik = 0
     while !(ok:=FindText(1079-150, 1304-150, 1079+150, 1304+150, 0, 0, Text))
     {


         if (ok:=FindText(1104-150, 1296-150, 1104+150, 1296+150, 0, 0, already_frnd))
        {
            dont_clcik = 1
            break
        }
     ...
     if dont_clcik = 0
    {
        X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
       MouseClick, left,%X%,%Y%
    Text:="|<add_req>*120$13.07U0nw9zYzkTwDy7z3zVzkzsTwDw7yHw80A0T"
    OutputDebug dd `n
     while !(ok:=FindText(1037-150, 1299-150, 1037+150, 1299+150, 0, 0, Text))
     {}
        X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
           MouseMove, %X%, %Y%
    Click down
   ...
   dont_clcik = 0
    ...
    OutputDebug cancel_button_window `n
    Sleep 100
     while !(ok:=FindText(1047-150, 1264-150, 1047+150, 1264+150, 0, 0, Text))
     {
        sleep 100
                    Send, {Esc down}
            sleep 180
...
acceptreq()
{
    ...
    motto:="|<motto>*90$17.00600400800k01U07U0D00z01z07y0Ts7z0Dy0Tw000008"

 while (ok:=FindText(0, 0, 3840, 2160, 0, 0, motto))
 {

    X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
    MouseClick, left,%X%,%Y%
    accept_icon:="|<accept_icon>*132$9.1sD1sTzsQ3114Q"

    while !(ok:=FindText(0, 0, 3840, 2160, 0, 0, accept_icon))
    {}
      X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
    MouseMove, %X%, %Y%
    Click down
    ...
    Click up
    sleep 100
    Text:="|<acceptguibutton>*131$12.zz0000yTyTyTyTyTyTyTyTyTyTyTyTyTyTyTU"

 while ! (ok:=FindText(373-150000, 339-150000, 373+150000, 339+150000, 0, 0, Text))
 {}
   X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
    MouseMove, %X%, %Y%
    Click down
...
notinlobby:="|<displaypic>*20$43.000000000000000000000000000000000000000000003zzzk007zzzy08"
    ...
    if (ok2:=FindText(450, 0, 3220, 1540, 0.01, 0.01, notinlobby))
    {
        ;OutputDebug `n
        ...
        for j in ok2
        {
            ...
            ,Yas :=ok2[j].y
			,acc_num := Xas/640
            ,acc_numx=Ceil(acc_num)
			,acc_num := yas/480
            ,acc_numy=Ceil(acc_num)
			;OutputDebug %acc_numx% , %acc_numy% `n
            copy_code_setup(acc_numx,acc_numy)
		}
        for i,v in ok2
        {
            ;detect which is not inlobby
...
copy_code(x,y)
{
	...
	while not Clipboard
	{
                ...
                MouseClick,left,-127,29,,40,, R
                sleep 500
                MouseClick,left,-145,120,,6,,R
                MouseClick,left,59,0,,1,,R
				sleep 400
	}
	codes.push(Clipboard)
	;OutputDebug % codes.MaxIndex()
    OutputDebug %Clipboard% `n
}
copy_code_setup(x,y)
{
	x := x * 640 - 20
