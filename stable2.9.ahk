...
variables()
    {
        ...
        read_value()
        IniRead,owner,stats.ini,SectionName,owner
        OutputDebug ac num %set%
        OutputDebug %steam_folder%
        sold_accs = 0
        UX = 1
        UY = 1
    ...
    if (lobbystatus2 != lobbystatus1)
      {
        ...
        OutputDebug %matchcount% done till ac num %set% , mode %my_mode%
        print("one is prime")
...
getscore()
  {	
    wins = 6 ; delete it only here for test
    if (t = "no")
        {
...
tenaccopen()
    {
      ...
      while (i <= total_accounts_open)
        ...
        if (errorlevel = 0)
            {
             ;OutputDebug found csgo @ position %i% in menu
            }
        else if (ok:=FindText(sx, sy, "150", "150", 0, 0, Text))
            {
              OutputDebug found csgo @ position %i% in game
              X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
              Click, %X%, %Y%
              Sleep 50
              Send, {l down}
