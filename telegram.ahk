...
Drive = C:\Users\%A_Username%\Google Drive\file share
if !FileExist(Drive)
...
global Users :=[] ,enter := "%0A", pc :=[] ,Drive,dailysummary
logins :=[]
...
;url := "https://api.telegram.org/bot" botToken "/sendMessage?text="A_ComputerName "'s " A_UserName " is up&chat_id=" chatID "&reply_markup=" keyb
...
hourlyupdate:
	text = ""
	IniRead, last_match, status.ini,SectionName , last_match
	...
	;				else
	;					{
	;						ini = \\%user%-PC\Users\%user%\Documents\15-15\status.ini
...
UpdateTimer:																									; checks constantly for user input in your bot
	...
	If oUpdates.ok																			; check if json answer was "ok" : true
	{
		...
		For key, msg in stack
		{
			...
			if oCustomers.Haskey(from_id)				; check for known users...   optional
			{
				...
				{
					tele := telev()
					IniRead,version,status.ini,variable,version
	...
	if a_hour = 00
	{
		if (dailysummary = 1)
		{
			...
			if pday = 0
				pday = 7
			IniRead,cdperday,data.ini,week,%pday%
			if (cdperday == "ERROR")
				cdperday = 0
			IniRead,matchesperday,status.ini,week,%pday%
			dailysummary = 0
			IniRead,set,status.ini,SectionName,set
			IniRead,totalaccounts,%Drive%\z_extra\work.ini,SectionName,totalaccounts
			totalaccounts++
			IniRead,yesterdayset,data.ini,info,yesterdayset
			IniWrite,%set%,data.ini,info,yesterdayset
			dailysummary = 0
			if set < yesterdayset
			{
				diff := totalaccounts - yesterdayset
				diff := set + diff
			}
			else
				diff := set - yesterdayset
			diff += 230
			StringTrimRight, diff, diff, 1
			text = %a_username%, %enter% match/day = %matchesperday%, set done = %diff%
			OutputDebug %text%
			Telegram_MsgBox(0,a,text)

			if 	(A_WDay = 1 and weeklysummary = 1)
			{
...
need_update(botToken, msg, from_id, mtext)
{
	...
	IfInString,A_UserName,2700
	{
		Loop , % users.MaxIndex()
		{
			...
			{
				ini = \\%user%-PC\Users\%user%\Documents\15-15\status.ini
				Iniwrite, 1, %ini%,SectionName , need_update
				puser = %puser% updating %user%	%enter%
			}
		}
	}
	Iniwrite, 1, status.ini,SectionName , need_update
...
sendphoto()
{
	IniRead,strings,status.ini,tele_notes,msg
	IniRead,SelectedFile,status.ini,tele_notes,pic
...
status()
{
	...
	this = %this% %A_UserName%>set=%set%,matches=%matchcount%,lastest=%last_match%
return this
}
