...
updis_16(the_count = 30)
{
  ...
  Loop, %the_count%
  {
	if (sup:=do_16_16("1"))
	{
		...
		StartTime := A_TickCount
		ElapsedTime := A_TickCount - StartTime			
		while (ErrorLevel != 0 and ElapsedTime < 1000)
			{
				ElapsedTime := A_TickCount - StartTime
				MouseClick,right,2880,25
				Send, {l down}
				sleep 180
				Send, {l up}
				sleep 100
		...
		WinGet, OutputVar, PID , A
		run wmic process where ProcessID=%OutputVar% CALL setpriority "realtime" ,,hide
		reconnect_16()
        StartTime := A_TickCount
        ElapsedTime := A_TickCount - StartTime
  ...
  if (ElapsedTime > 11000) 
  	...
  	run wmic process where ProcessID=%OutputVar% CALL setpriority "idle" ,,hide
  weeee := ok.1.id
  OutputDebug msg %weeee% in  %ElapsedTime% , overtime =%overtime%
              if (ok:=FindText(2582-20, 777-100, 2582+20, 777+100, 0.01, 0.01, new_settings))
                            return    
if (overtime = 5  or overtime = 9 )
{
	if (my_mode = 16)
	{
	overtime = 0
  ...
  Loop, %the_count%
{
   ...
   ImageSearch, FoundX, FoundY, 2560, 0, 3200, 720,*5 reconnect.png
            StartTime := A_TickCount
          ElapsedTime := A_TickCount - StartTime			
  while (ErrorLevel != 0 and ElapsedTime < 2000)
     {
      ElapsedTime := A_TickCount - StartTime
MouseClick,right,2880,500
sleep 180
Send, {l down}
sleep 180
Send, {l up}
...
while(!ok:=FindText(2600, 424, 2740, 450, 0.15, 0.15, Text))
  {
    ...
    if (ElapsedTime > 11000) 
  	...
  	run wmic process where ProcessID=%OutputVar% CALL setpriority "idle" ,,hide
  weeee := ok.1.id
  OutputDebug msg is %weeee% in %ElapsedTime% overtime =%overtime%
...
reconnect()
  ...
  reconnect_16()
  {                  
    ImageSearch, FoundX, FoundY, 2560, 0, 3200, 960, *5 ok.png
     if (ErrorLevel = 0)
        {
          MouseClick, left, x, y ,,10
          MouseMove, 20, 40, 10, R
