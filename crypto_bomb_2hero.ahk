...
activeheroes = 2
heroarrow:="|<>0xEFCE9F@1.00$7.zzzzy"
,greenhealth:="|<greenhealth>0x8DC1B7@1.00$3.rU"
,close_hero_window:="|<close_hero_window>0xFFFBF5@0.95$23.y00zw01zz0Tzy0zzw1zXyDs7wTk3zy07zw0Dzs03y007w01zz03zy07zw0zXy1z7wTs3zzk7zzUDzs03zk07s"
,ntwrking:="|<ntwrking>0x82C188@1.00$49.zzzzzzzyTzzzzzzzDzzzzzzzzzw00UDzzzw00E7zzzw0083zz00000000Tz0020zzs"



if (ok:=FindText(0, 0, 1920, 1080, 0, 0, heroarrow))
    {
            X:=ok[1].x, Y:=ok[1].y
            Clickl( X, Y)
            sleep 1200
            Y:=ok[1].y - 10
            Clickl( X, Y)
             while (!ok:=FindText(0, 0, 1920, 1080, 0, 0, ntwrking))
            sleep 200
            X:=ok[1].x - 80, Y:=ok[1].y + 50
            MouseMove,x,y
            sleep 200
            clicked = 0
            loop,4
            {
                if (ok:=FindText(0, 0, 1920, 1080, 0, 0, greenhealth))
                {
                    for i,v in ok
                    {
                        if clicked = %activeheroes%
                                break
    ...
    Sleep 500
    if (ok:=FindText(0, 0, 1920, 1080, 0, 0, close_hero_window))
    {
            X:=ok[1].x, Y:=ok[1].y
            Clickl( X, Y)
            sleep 800
...
;    ,MyTicks1 := ElapsedTime/1000
;    ,MyTicks1:= SubStr(MyTicks1, 1, 4)
;    MyTicks:= SubStr(MyTicks, 1, 4)
;            OutputDebug,  %MyTicks1%/300 sec OR %MyTicks%/5 mins `n
;            sleep 1000
;}

ExitApp

Clickl(p1,p2,pp1 = 3,pp2=3)
{
    OutputDebug, %p1%,%p2%
