...
FileDelete coordinate.txt


MouseClick,left, 620,128
MouseClick,left,-127,29,,10,, R
...
Loop
{
	if (x != 0)
	{
		...
		{
			one_time = 1
	Text = MouseClick,left,%x%,%y%,,0,,
	FileAppend, %Text% `n,coordinate.txt
	newx := x
	x1:=x,y1:=y
		}
	}
	...
	if (newx != x)
	{
		if (one_time = 1) ;!FileExist ("C:\Users\rasti\OneDrive\Documents\15-15\coordinate.txt")
		{
		x2 := x - x1
		y2 := y - y1
		
		Text = MouseClick,left,%x2%,%y2%,,0,,R
		FileAppend, %Text%`n,coordinate.txt
		x1:=x,y1:=y
		,newx := x
		}
