...
Loop
{
if(ok:=FindText(2247, 1296, 150000, 150000, 0.1, 0.1, Text))
{
OutputDebug ya
}
else
{
  OutputDebug na
;Run, %A_ScriptDir%\IrfanView\i_view64.exe /capture=1 /convert=test\%A_TickCount%.png
}}
if (ok:=FindText(1037, 887, 15000, 15000, 0, 0, Text))
{
  CoordMode, Mouse
...
Text:="|<unranked>23@1.00$71."

if (ok:=FindText(833, 2015, 15000, 15000, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
   Click, %X%, %Y%
...
OutputDebug additional lobby check
pidListFromName("csrss.exe") 
For Process in ComObjGet("winmgmts:\\.\root\cimv2").ExecQuery("Select * from Win32_Process [color=red]WHERE NAME='CHROME.EXE'[/color]")
{
	Process.GetOwner(UserName, UserDomain)
	MsgBox % Process.Name "`n" UserName "`n" UserDomain "`n" Process.commandline
}
...
{
	vOutput := objProcess.GetOwner(strNameOfUser)
    OutputDebug %vOutput%
