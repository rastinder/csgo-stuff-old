...
reset_timer := A_TickCount - ultra_start_Time
	...
	FileGetTime, globalfile1 ,\\2700-pc\C\Users\Admin\Documents\15-15\currentuser.txt
	FileGetTime, globalfile2 ,\\2700-pc\C\Users\2700_1_1\Documents\15-15\currentuser.txt
	FileGetTime, globalfile3 ,\\2700-pc\C\Users\2700_1_2\Documents\15-15\currentuser.txt
	FileGetTime, localfile , currentuser.txt
				if (globalfile1 > localfile or globalfile2 > localfile or globalfile3 > localfile )
				{
					logibs :=[]
					FileDelete,currentuser.txt
					loop, Read,\\2700-pc\C\Users\Admin\Documents\15-15\currentuser.txt
							logibs.push(A_LoopReadLine) ; 
					loop, Read,\\2700-pc\C\Users\2700_1_1\Documents\15-15\currentuser.txt
							logibs.push(A_LoopReadLine) ; 
					loop, Read,\\2700-pc\C\Users\2700_1_2\Documents\15-15\currentuser.txt
							logibs.push(A_LoopReadLine) ; 
					loop, % All_UserInfo.Maxindex()
					{
						eh := All_UserInfo[A_Index]
						,eh := StrSplit(eh, ",")
						,eh := eh[1]
						v := HasVal(logibs,eh) 
						OutputDebug  v=%v%
					...
					;loop, % All_UserInfo.Maxindex()
					;{
					 ;sa:=All_UserInfo[A_Index] 
					;FileAppend,%sa% ,currentuser.txt
					;}
				}
...
;	}
 ...
 if (ok:=FindText(0, 0, 3200, 960, 0.00, 0.01, Text))
    ...
    if (lobbystatus2 = 1 and lobbystatus1 = 1 and my_mode = 15)
      {
	...
	if( runfuntion = 0)  ;check must
     {  
		...
		odebug = ct= cts"%cts%" copy"%clipboard%" real"%getct%"  
				TargetFileServer = /crop=(%sx%,%sy%,17,12)
		Run, %A_ScriptDir%\IrfanView\i_view64.exe /capture=1 %TargetFileServer% /silent /convert=%A_ScriptDir%\score\%getct%__%A_TickCount%.png
...
;      odebug  ------------
      ...
      ;tss += 0
		...
		TargetFileServer = /crop=(%sx%,%sy%,17,12)
		Run, %A_ScriptDir%\IrfanView\i_view64.exe /capture=1 %TargetFileServer% /silent /convert=%A_ScriptDir%\score\%gett%__%A_TickCount%.png
		if (my_mode = 16)
		{
