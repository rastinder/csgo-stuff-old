   ...
   ;               wins = 6
    ;            }
     ;       else
      ...
      tss = 0
      matchcount++
      outputdebug `n  match %matchcount% completed of set=%set%
	  lobbycount = 0
	  ...
	  Text:="|<cooldown>*149$41.00000007014000E0280011GZGfJ248AcKe29GNGPI7iQiQYc00000000000004"
	   if (ok:=FindText(0, 0, 3200, 1920, 0, 0, Text))
          {
			outputdebug `n cd lag geya  yellow wala
			sleep 100
			forcere = 1
			swap_accs = 0
			changeaccs()
...
if (reset_timer >= 960000 and my_mode != "pr2") ; 1800000
	{
		Reload
                              ...
                              while(ElapsedTime < rounddelay )
                                      {
                                        ...
                                        if (t = "no" and ElapsedTime > rounddelay * 0.8)
                                          {
                                              error()
                                              outputdebug `n chk score %ElapsedTime%
                                              getteam()
                                                  runfuntion = 0
                                                  getscore()
                                                  runfuntion = 1
    ...
    if (firstrun = 1)
      {
        ...
        ;outputdebug `n checking map
        outputdebug `n map screen ;errorlevel = %ErrorLevel%
        StartTime := A_TickCount
        ,ElapsedTime := A_TickCount - StartTime
		if (!ok:=FindText(0, 0, 3200, 1920, 0, 0, Text))
			 outputdebug `n not a map screen
        while (!ok:=FindText(0, 0, 3200, 1920, 0, 0, Text))
               {
    ...
    else
    {
      ...
      ;outputdebug `n map errorlevel = %ErrorLevel%
		...
		if (halftime = 0)
			...
			halftime = 0
	OutputDebug diff=%diff%,whiteTime-%whiteTime%,joinTime-%joinTime% `n ;too_late-%too_late% `n
	OutputDebug reconnectdelay=%reconnectdelay%`n
	sleep 200
  }
}
reconnecterror()
                    ...
                    if (onetime = 0)
                      {
                        ...
                        outputdebug `n scoreboard found
...
;	{
        ...
        if (ok:=FindText(3055-15, 157-15, 3055+15, 157+15, 0, 0, Text) && lobbycount <= 2 && swap_accs = 0)
          {
			lobbycount++
			outputdebug `n normal invite initiated lobbycount=%lobbycount%
...
;	}
    ...
    if (lobbystatus2 != lobbystatus1)
      {
        outputdebug `n one is prime other is non prime  %matchcount% done till ac num %set% , mode %my_mode%
		if (swap_accs != 1)
		{
    ...
    if (lobbystatus2 = 1 and lobbystatus1 = 1)
      {
        outputdebug `n both prime, %matchcount% done till ac num %set% , mode %my_mode%
...
update_client_check(dhkka)
{
	...
	if (ok:=FindText(0, 0, 2560, 960, 0, 0, Text) or play_now = 1)
	{
		...
		MouseMove x,y
		outputdebug `n client check %play_now%
		Sleep 100
		play_now = 0
		...
		while(x = 0)
		{
			...
			loop
			{
               ...
               ; ControlSend,,{Esc},csgo.exe
                ...
                if (ok:=FindText(2103-700, 1171-750, 21037+700, 1171+750, 0.1, 0.1, Text))
				{ 
					outputdebug `n win 10 play game clicked
					X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
					MouseClick, left,%X%, %Y%,1,50,,
					sleep 500
					y := y + 60
...
;	else
;		skiphftm = 1
		outputdebug `n rounds %total% mode %my_mode% do1614 %do1614%
		;do1614 = 1 ; testing
		if (do1614 = 1)
		{
			if ( cts = 13)
			...
			{
				IniRead, reconnectdelay, status.ini,SectionName , reconnectdelay
				outputdebug `n doing 1614
				my_mode = 16
;				skiphftm = 1
				return
			}
		}
		;outputdebug `n NOT doing 1614
   ...
   FileRemoveDir, %c%, 1
		...
		Loop{
			...
			if (ErrorLevel = 0){
				...
				if (v = 1){
					set := set + 10
					outputdebug `n %set% set is prime
					Iniwrite, %matchlimit%, status.ini,match logs , set_%set%
				}
				else{
					break
		...
		Start_Time := A_TickCount
		wait_time := A_TickCount - Start_Time
		while (x = 5 and wait_time < 1 ) ; eda matlab wait kro for 
		{
			wait_time := A_TickCount - Start_Time
			error()
			ControlClick , OK, VAC bypass
...
tenaccopen()
	{
	  ...
	  ,Text:="|<in_match_check>0xFA2E2A@0.71$6.zznkyT3nzzU"
        ...
        ,ty := ty%b%
			...
			{
				outputdebug `n found csgo @ position %i% but in-game
				X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
				Click, %X%, %Y%
				Sleep 50
				Send, {l down}
				...
				else
				{
					...
					if (ok:=FindText(0, 0, 3200, 960, 0, 0, Text))
						{
							if ( ok.MaxIndex() <= 4)
							{
								outputdebug `n 4 to ghat acc opened c
                  ...
                  ;Run	%steam_folder%steam.exe -inhibitbootstrap -single_core -login %users% -applaunch 730 -silent -novid -nocache -nohltv -softparticlesdefaultoff -nopreload -hijack -limitvsconst  +mat_disable_fancy_blending -r_emulate_g -refresh 10 -nominidumps -safe -noforcemparms -nomsa -nojoy -noaafonts -nosound -threads 1 -noshader -safe  -d3d9ex -nocrashdialog -h 480 -w 640  %p% -low 
					...
					while (errorlevel != 0)
					{
						...
						{
							outputdebug `n trying to open 2nd time
							RunWait taskkill /PID %steam_pid% /F /T
							Run,steam.exe %arguments% -login %ha% %p%  -low  ,%steam_folder%,,steam_pid
							sleep  10000
							run wmic process where ProcessID=%steam_pid% CALL setpriority "idle" ,,hide
						...
						else if (timer = 60)
						{
							...
							forcere = 1
							outputdebug `n unable to open in time = %timer%
							;set := set - total_accounts_open
							changeaccs()
							IniRead, matchcount, status.ini,match logs , set_%set%
							return
					...
					outputdebug `n opened in timer = %timer%
...
check_rank( )
	  ...
	  Loop, Files, \\2700-pc\c\DLL\rank\*.png
	{
		...
		for index, element in myrank
			{
			ImageSearch, FoundX, FoundY, %sx%, %sy%, %tx%, %ty%, *5 \\2700-pc\c\DLL\rank\%element%.png
			if (errorlevel = 0)
			{
			...
			if (errorlevel = 1)
			{
				...
				;Run, %A_ScriptDir%\IrfanView\i_view64.exe /capture=1 %TargetFileServer% /silent /convert=\\2700-pc\c\DLL\rank1\%A_UserName%_%rand%_%A_TickCount%.png
				Sleep 2000
			}
			for index, element in mypr
			{
			ImageSearch, FoundX, FoundY, %sx%, %sy%, %tx%, %ty%, *10 \\2700-pc\c\DLL\pr\%element%.png
			if (errorlevel = 0)
			{
				If InStr(element,"black")
					break
			...
			if (errorlevel = 1)
			{
				...
				;Run, %A_ScriptDir%\IrfanView\i_view64.exe /capture=1 %TargetFileServer% /silent /convert=\\2700-pc\c\DLL\pr1\%A_UserName%_%rand%_%A_TickCount%.png
				Sleep  2000
			}
			i++
			,a++
		...
		outputdebug `n %countr% ranks and, %countp% new pr found
	}
click_settings_then_video()
	{
		outputdebug `n video settings
		Text:="|<setting>*127$16.zzzwDzkzv3T00w03kwDXlyD7kwD00w03v3TwDzkzzzy"
		while (ok:=FindText(0, 0, 3200, 1920, 0, 0, Text))
		{
			X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
		...
		{
			outputdebug `n video settings on high
			Text:="|<csgo>*116$26.zzzzzzzzzzzzzzzzzzzzzzzzzzzzDzzzUTzzk7zzw3zzz3zwkkkl0A88C320vkK4C41U0DyPzzziTzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzs"
			while (ok:=FindText(0, 0, 3200, 1920, 0, 0, Text))
				{
					X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
			...
			RunWait, video.bat, %steam_folder%userdata
			outputdebug `n video settings fixed
  ...
  ;Text:="|<expire_rank>148@0.96$22.007U001000G00V4024E08V00Y0010U034U"
			...
			outputdebug `n matchlimit = %matchlimit% & do1614 = %do1614% ,,
