...
;Text:="|<ct_or_T_win_logo>*171$9.zxzzxVhhhhhhzw"
...
Gui, Show, x3250 y10 NoActivate  ; NoActivate avoids deactivating the currently active window.
...
global lobby,loading,win,script,script1,go,firstrun,teamwin,wins,join,error1,a1,a2,reconnectdelay,warmupdelay,halftimedelay,halftime,rounddelay,UX,UY,reconnectdelay1,reconnectdelay2,my_mode,rounds,xc,yc,l,sx1,sx2,sx3,sx4,sx5,sy1,sy2,tx1,tx2,tx3,tx4,tx5,ty1,ty2,get,getu,getd,ts,t,ct,cts,tss,matchcount,skiphftm,fixscore,s,godownfound,godupfound,nextmatch,set,Primes_StartingUser,matchlimit,getct,gett,rank_open,play_now,click_time,click_time1,time_since_last_match,ha,runfuntion,test1,total_accounts_open,i_value,steam_folder,owner,swap_accs,do1614,steampids,odebug,forcere,lobbycount,inv_code,fix_rounddelay,All_UserInfo,need_mode
...
variables()
    {
        ...
        Else If (ErrorLevel1 = 0)
        ...
        else
          {
            ...
            Text:="|<new_settings>0xFFFFFF@1.00$12.84M6QCHm1U1UU"
	...
	if (need_update = 1)
		{
			...
			Run Taskkill /FI "USERNAME eq %USERNAME%" /IM telegram.exe /F /T
			ExitApp
		}
    if ( join = 1 )
      {   
...
swap_current_accounts(this,that){
	this_array := StrSplit(this, A_Space, ".")
	,that_array := StrSplit(that, A_Space, ".")
	loop, % this_array
...
,new_settings:="|<new_settings>0xFFFFFF@1.00$12.84M6QCHm1U1UU"
...
loop, 2
			{
				OutputDebug click reconnect %A_Index% `n
					x1 := placing("c" , "x",A_Index)
					,y1 := placing("a" , "y",A_Index)
					,x2 := placing("b" , "x",A_Index)
					,y2 := placing("c" , "y",A_Index)			
					...
					while (ErrorLevel = 0)
						...
						OutputDebug reconnected %A_Index% `n
					if a_Index = 1
						disthe = 2
					else 
						disthe = 1
					OutputDebug disnum %disthe%  wait_time %wait_time%`n
					onetime = 1
					...
					while (ElapsedTime < wait_time)
						{
							ElapsedTime := A_TickCount - StartTime
							if (onetime = 1 and ok:=FindText(2600, 904, 2740, 930, 0.15, 0.15, Text))
								{
									OutputDebug join found try to disconnect %disthe%  `n
									ElapsedTimeforjoin := A_TickCount - StartTimeforjoin
									,x1 := placing("c" , "x",disthe)
									,y1 := placing("a" , "y",disthe)
									,x2 := placing("b" , "x",disthe)
									,y2 := placing("c" , "y",disthe)
									,StartTime1 := A_TickCount
									,ElapsedTime1 := A_TickCount - StartTime1
									,onetime := "0"
									ImageSearch, FoundX, FoundY, %x1%, %y1%, %x2%, %y2%, *5 reconnect.png
									while (ErrorLevel != 0 and ElapsedTime1 < 3000)
										{
						...
						Click up
											...
											if ElapsedTimeforjoin > 3000
												OutputDebug time per round = %ElapsedTimeforjoin%
												;wait_time := ElapsedTimeforjoin + ElapsedTimeforjoin/2
											StartTimeforjoin := A_TickCount
											,ElapsedTimeforjoin := A_TickCount - StartTimeforjoin
										}
									OutputDebug re err %ErrorLevel% @ %disthe% `n
								}
						}
						OutputDebug w8 over is %disthe% disconnected? %disconnect_done% `n
						if (disconnect_done ==  false and ok:=FindText(50, 425, 730, 460, 0, 0, map_check))
							{
								OutputDebug wait for join_msg `n
								while(!ok:=FindText(2600, 904, 2740, 930, 0.15, 0.15, Text))
								{
									ElapsedTime := A_TickCount - StartTime
									if (ok:=FindText(2560, 0, 3200, 1920, 0, 0, halftime1))
									{
										OutputDebug halftime join skip
										break
									}
									else if (ElapsedTime > 9000){
										OutputDebug cant find join `n
										break
									}
										
								}
								ElapsedTime := A_TickCount - StartTime
								OutputDebug gave up on join_msg after %ElapsedTime% lets dis %disthe%  `n
								MouseClick,right,x1,y2
						sleep 50
						Click down
						sleep 50
				...
				if (ok:=FindText(0, 0, 2560, 460, 0.15, 0.15, in_match_check)) { 
					OutputDebug in match found
				for i,v in ok
					{
...
scoreboard()
	{
		scoreboard:="|<scoreboard>0xFDFDFD@0.29$5.1w07+8EU8" ;scoreboard
...
error()
    {   
        ...
        if ( loading != 1 ) ; loading one means match wich aa
          {
			  Text:="|<trusted_continue>*109$35.X7sTyk490250dEG0+NmUY0I2Z180dV2GEUEU"
              if (ok:=FindText(0, 0, 3200, 1920, 0, 0, Text))
				{ 
					gos()
					for i,v in ok
              ...
              while (ErrorLevel = 0)
                      {
                          ...
                          MouseMove, xpos, ypos
                          BlockInput, off
                          ImageSearch, FoundX, FoundY, 0, 0, 3200, 1920, *25 ok2.png
                      }
              }
          }   
        }
    ...
    while (x = 0)
            {
			  ...
			  if changeaccs()
			  return
              ;TText:="|<new_settings>0xFFFFFF@1.00$12.84M6QCHm1U1UU"  ; are we on home screen?
              Text:="|<new_settings>0xFFFFFF@1.00$12.84M6QCHm1U1UU"
              if (ok:=FindText(2582-16, 777-100, 2582+16, 777+100, 0, 0, Text))
                  {
                    X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
...
lobbycount = 3
        ...
        if (ok:=FindText(3055-15, 157-15, 3055+15, 157+15, 0, 0, Text) && lobbycount <= 2 && swap_accs = 0)
          {
			lobbycount++
            ...
            Sleep %zzz%
			sleep 3000
	  ;Text:="|<pending_inv_check>*118$9.zzzzz7kSvrTnwTbxzjzzjxzzzzzU"
	  ;if(ok:=FindText(0, 0, 2560, 960, 0.01, 0.01, Text))
...
;	}
    ...
    if (ok:=FindText(0, 0, 3200, 1920, 0, 0, Text))
      ...
      popout()
	  ;Text:="|<map_check>*118$9.zzzzz7kSvrTnwTbxzjzzjxzzzzzU"
	  ;if(ok:=FindText(0, 0, 2560, 960, 0.01, 0.01, Text))
		Text:="|<pending_joins>*210$48."
 if (ok:=FindText(0, 0, 3200, 960, 0.00, 0.01, Text))
		{
    ...
    if (lobbystatus2 != lobbystatus1)
      {
		...
		if (swap_accs != 1)
		{
			IniWrite, %lobbystatus1%_%lobbystatus2% , status.ini,prime_status, prime_%set%
			home()
			Text:="|<new_settings>0xFFFFFF@1.00$12.84M6QCHm1U1UU" ; are we home screen
			if (ok:=FindText(0, 0, 3200, 1920, 0.1, 0.1, Text))
				{
					for i,v in ok
						{
    ...
    if (lobbystatus2 = 1 and lobbystatus1 = 1 and my_mode = 15 and medal != 1)
      {
		Text2:="|<rank_expired>*116$59.zRzzzzzzzzznzzzzzzzzz7zzzzzzzzyTzzzzzzzzxzzzy7zzzzvzzzk3zzzzzzzzkDzzzzjzzzUTzzzzTzzy0Tzzzzzzzy1zzzzzzzzw3zzzzzzzzs7zzzzzzzzzzzzzzzzzzzzzzy"
		if (ok:=FindText(0, 0, 3200, 960, 0, 0, Text2 ))
			{
				odebug = rank expired
...
lobbycheck()
  {
	...
	,Text:="|<lobby_check_adv>0xD8F7FE@0.94$9.kM0G80EA"
	ok:=FindText(300, 10, 3200, 580, 0, 0, Text)
	  if ( ok.MaxIndex() != 10)
	  {
    sx1 := "0"
   ,sy1 := "0"
	...
	{}
		else
		{
			...
			if (globalfile > localfile)
			{
				...
				RunWait Taskkill /FI "USERNAME eq %USERNAME%" /IM telegram.exe /F /T
				run, autostart.exe,%a_scriptdir%
				Run Taskkill /FI "USERNAME eq %USERNAME%" /IM stable.exe /F /T
				Run Taskkill /FI "USERNAME eq %USERNAME%" /IM host.exe /F /T
				Run Taskkill /FI "USERNAME eq %USERNAME%" /IM telegram.exe /F /T
				ExitApp
			}
...
check_rank( )
	  ...
	  Loop, Files, %folder%\z_extra\rank\*.png
	{
		...
		for index, element in myrank
			...
			if (errorlevel = 5)   ;errorlevel = 1 testing
			{
				countr++
			...
			{
			ImageSearch, FoundX, FoundY, %sx%, %sy%, %tx%, %ty%, *10 %folder%\z_extra\pr\%element%.png
			if (errorlevel = 0)
			{
				If InStr(element,"black")
					break
...
click_settings_then_video()
	...
	mode_change()
	{
		if (my_mode != 16)
		{
			...
			;Text2:="|<expire_rank>148@0.96$22.007U001000G00V4024E08V00Y0010U034U"
			;if (ok:=FindText(0, 0, 3200, 960, 0.1, 0.1, Text2))
			;	{
			;my_mode  = 15
			;oDebug = mode=%my_mode% till=%matchlimit% & do1614 = %do1614%
			;return
			;	}
			if (ok:=FindText(0, 0, 3200, 960, 0.1, 0.1, Text))
			{
				newmatchcount := matchlimit - 3
			...
			oDebug = matchlimit>%matchlimit%, do1614>%do1614%
...
; ==============================================================================================
	...
	pidListFromName(name) {
		...
		{
			hr := objProcess.GetOwner(Process.processId)
			MsgBox % hr
