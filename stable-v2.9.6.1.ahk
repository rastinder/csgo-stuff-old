...
; Text:="|<scoreboard>*90$11.U2w1k3Vb7CCQQmNgnNc0k"
...
;SendMode Input
        ...
        WinGetTitle, Var, Steam - Pending Cloud Uploads
        OutputDebug %var% `n  
        If InStr(var,"Cloud")
                    {
...
play_pr2()
{
  ...
  Loop
  {
    ...
    while (ok:=FindText(0, 960, 3200, 976, 0, 0, Text))
    {
    ...
    while (ok:=FindText(0, 16, 3200, 976, 0, 0, Text))
    {
      X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
      Click, %X%, %Y%
    }
    ...
    ;if (ok:=FindText(0, 16, 3200, 976, 0, 0, Text))
    ;   {
    ;      X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
    ;      MouseClick, left, %X%, %Y%
    ;      sleep 1600
    ...
    while ( ElapsedTime <= 2400000 ) ;........................................................................................................... loop start
    {
      ...
      while (ok:=FindText(0, 16, 3200, 976, 0.1, 0.1, Text))
      {
...
variables()
{
  ...
  IniRead,swap_accs_disable,status.ini,SectionName,swap_accs_disable
  IniRead, do1614, status.ini,SectionName , do1614
    if do1614 = ERROR
       Iniwrite,0,status.ini,SectionName,do1614
  if swap_accs_disable = ERROR
    Iniwrite,0,status.ini,SectionName,swap_accs_disable
  ...
  else
  {
    ...
    if (ok:=FindText(2566, 677, 2598, 877, 0, 0, Text))
    {
      OutputDebug `n ys
    }
    else
    ...
    if ( loading = 1 )
    {
      ...
      else
      {
        ...
        else if (my mode = 16)
        {
                        dis_8_acc("down")         
                        super_dis("down")       
                         ;   dis_8_acc(t) ;............or ("down") or ("up")dis_8_acc(ct)   super_dis(ct) 
                        ;  super_dis(t) 
      }
       Text:="|<new_settings>*119$14.VsQSD7XVsM0601q6zVzsTzzs"
    while (!ok:=FindText(2566, 677, 2598, 877, 0, 0, Text))
    {
      X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
      Sleep 1000
    } 
        ...
        OutputDebug `n match %matchcount% completed
                ...
                {
                   OutputDebug cd lag geya yellow wala
                    sleep 100
                    matchcount := matchlimit++
                    swap_accs = 0
                    changeaccs() ; fgfgfg
...
super_dis(which)
{
    ...
    loop,
    {
        ...
        while (ElapsedTime < reconnectdelay) ;............................................................................................................................................wait_time,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,
        {
            ...
            if scoreboard()
            {
                outputdebug out from scoreboard
                return
            }
...
scoreboard(forceout = 0)
{
    ...
    ,in_match_check:="|<in_match_check>0xFA2E2A@0.71$6.zznkyT3nzzU"
    ,map_check:="|<map_check>0xF8D764@1.00$13.6A2231bUzkTsDw7y3z1zUwkM8844E"
e (ok:=FindText(288, 482, 2859, 975, 0.01, 0.01, scoreboard) or forceout > 0)
    {
        if (ok:=FindText(288, 482, 2859, 975, 0.01, 0.01, scoreboard))
            outputdebug scoreboard found
        for i,v in ok
        {
...
reconnect_winteam(which)
{
    ...
    if (ok:=FindText(x1, y1, x2, y2, "0","0", reconnect_r)) ;.................................................................................................................................reconect.............................................................................................
    {
        outputdebug reconnect in win lobby found
        for i,v in ok
        {
  ...
  start_check()
  {
    ...
    while (ok:=FindText(0, 16, 3200, 3200, 0, 0, Text))
    {
      X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
      Click, %X%, %Y%
      Sleep 100
  ...
  error()
  {
    ...
    if ( loading != 1 ) ; loading one means match wich aa
    {
      ...
      if (ok:=FindText(0, 16, 3200, 976, 0.1, 0.1, Text))
      {
  ...
  map()
  {
    ...
    if (firstrun = 1)
    {
      ...
      ;OutputDebug checking map
      OutputDebug map screen errorlevel = %ErrorLevel%
      StartTime := A_TickCount
      ,ElapsedTime := A_TickCount - StartTime
      if (!ok:=FindText(0, 0, 3200, 1920, 0, 0, Text))
        OutputDebug not a map screen
      while (!ok:=FindText(0, 0, 3200, 1920, 0, 0, Text))
      {
    ...
    else
    {
      ...
      ImageSearch, FoundX, FoundY, 0, 0, 3200, 960, *8 universal_map2.png
...
{ 
  ...
  while (ErrorLevel = 0)
  {
    outputdebug ok in reconnect() 2873
...
{ 
  ...
  while (x = 0)
  {
    ...
    if (ok:=FindText(1403, 1021, 2803, 1321, 0.15, 0.15, Text))
    { 
      X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  ...
  if (lobbyfull = 1)
  {
    ...
    while (ok:=FindText(2499,87,2799,607, 0, 0, Text))
    {
      X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
      Click, %X%, %Y%
    }
    ...
    if (ok:=FindText(2756,23,2856,123, 0, 0, Text))
    {
      lobbystatus1 = nonprime
      OutputDebug `n %lobbystatus1%
    }
    if (ok:=FindText(2756,503,2856,603, 0, 0, Text))
    {
      lobbystatus2 = nonprime
...
pressaccept()
{
   loop,2
    ...
    ok:=FindText(2722, 200, 3050, 787, 0, 0, Text)
    OutputDebug % ok.MaxIndex() "total accept"
    if ( ok.MaxIndex() = 16)
    {
        loop 2
        {
            MouseClick,left,320,230,,0,,
            loop,4
            {
                MouseClick, left,640,0,,0,, R
            }
            MouseClick, left,320,710,,0,,
            loop,5
            {
                MouseClick, left,640,0,,0,, R
                Sleep 30
...
update_client_check(dhkka)
{
  ...
  if (ok:=FindText(0, 16, 3200, 1920, 0, 0, Text) or play_now = 1)
  {
    MouseMove x,y
    ...
    while(x = 0)
    {
      ...
      loop
      {
        ...
        if (ok:=FindText(2103-700, 1171-750, 21037+700, 1171+750, 0.1, 0.1, Text))
        { 
          outputdebug win 10 play game clicked
          X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
          MouseClick, left,%X%, %Y%,1,50,,
          sleep 500
          y := y + 60
    ...
    FileDelete, %steam_folder%steamapps\common\Counter-Strike Global Offensive\csgo\cfg\%map_name%.cfg
    FileDelete, %steam_folder%steamapps\common\Counter-Strike Global Offensive\csgo\cfg\ma
    IniWrite, %map_command% %map_name% , %steam_folder%steamapps\common\Counter-Strike Global Offensive\csgo\cfg\%map_name%.cfg,echo ,
    IniWrite, %competitive% competitive , %steam_folder%steamapps\common\Counter-Strike Global Offensive\csgo\cfg\%map_name%.cfg,echo ,
    IniWrite, host_writeconfig , %steam_folder%steamapps\common\Counter-Strike Global Offensive\csgo\cfg\%map_name%.cfg,host ,
  ...
  if (ok:=FindText(0, 16, 3200, 1920, 0, 0, Text) or play_now = 1)
  {
    ...
    while(x = 0)
    {
      ...
      loop
      {
        ...
        Text:="|<rety_connection>*124$71.zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzy71bHg661MAvtotCXPtrSb9rrvuhKrryxTJjjrp+pUjxuydTSjf5XTSvpxMyRCL/ayRrdmty71iLg67jMBns"
        if (ok:=FindText(0, 16, 3200, 1920, 0, 0, Text))
        {
          play_now = 1
          update_client_check("0") 
          return
    ...
    while (x = 0)
    {
      ...
      Text:="|<new_settings>*119$14.VsQSD7XVsM0601q6zVzsTzzs"
      if (ok:=FindText(1603, 671, 2603, 1671, 0, 0, Text))
      { 
        X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
        sleep 500
        y := y + 60
...
getteam()
{
  ...
  if (ok:=FindText(3010, 346, 3330, 466, 0, 0, Text))
  {
    t = up
...
{
  if (ok:=FindText(3010, 825, 3330, 945, 0, 0, Text))
  {
    t = down
    OutputDebug `n t.gun t = %t%
    get := ""
...
home()
{
  ...
  if (ok:=FindText(0, 16, 3200, 976, 0, 0, Text))
  {
    ...
    while (ok:=FindText(2484, 16, 2684, 976, 0, 0, Text))
    {
      X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
      Click, %X%, %Y%
    }
...
if (ok:=FindText(1809, 977, 2109, 1277, 0, 0, Text))
{
  play_now = 1 
  update_client_check("0")
  return
...
while (x = 0 and wait_time < 300000 ) ; eda matlab wait kro for 
{
  ...
  Text:="|<close1>*70$22.bww49U03aEXCN04dYEsk40U"
  while (ok:=FindText(0, 16, 3200, 976, 0, 0, Text))
  {
    X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
    Click, %X%, %Y%
  ...
  if (ok:=FindText(1886, 525, 2186, 1525, 0, 0, Text))
  {
    play_now = 1 
    update_client_check("0")
  return
...
if (ok:=FindText(328, 16, 3200, 976, 0, 0, Text))
{
  SET := SET - total_accounts_open
  newchangeaccs()
  break
...
if (ok:=FindText(1971, 1096, 2271, 1396, 0.1, 0.1, Text))
{
  play_now = 1 
  update_client_check("0")
  return
...
Text:="|<new_settings>*119$14.VsQSD7XVsM0601q6zVzsTzzs"
if (ok:=FindText(x1, y1, "323", "248", "0.1", "0.1", Text))
{
...
csgo_size_check()
{
  ...
  if (ok:=FindText(0, 16, 3200, 3200, 0, 0, Text))
  {
    X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
    Click, %X%, %Y%
    sleep 100 
...
placing(one , two , num := "10" ) ; one(a = start point b = end point c = center point) two (x or y)
{
  place_1_a_x := "3" 
  ,place_1_b_x := "643"
  ,place_1_c_x := "323"
