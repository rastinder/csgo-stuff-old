...
;                    }
                ...
                UX = 0
                UY = 0
                Text:="|<cooldown>0xE1C111@1.00$10.VwwHUCEt81U"
                if (ok:=FindText(0, 0, 3200, 1920, 0, 0, Text))
                {
...
UpdateOSD:
    ...
    else if (reset_timer >= 9650000000000 and my_mode != "pr2") ; 1800000 ; 965000
    {
...
super_dis(which)
{
    ...
    ,win:="|<ct_or_T_win_logo>*171$9.zxzzxVhhhhhhzw"
    ,cooldown:="|<cooldown>0xE1C111@1.00$10.VwwHUCEt81U"
...
abandon_function(abandon_num)
{
        ...
        sleep 980
        abandonbt:="|<abandon>*88$28.Y980+EYU8d2E8U0U0W1UMU0c"
        if(ok:=FindText(0, 0, 4096, 4096, 0.05, 0.05, abandonbt))
            {
                X:=ok.1.x, Y:=ok.1.y
                MouseClick, left, x, y
            }
        sleep 980
        ok4on:="|<ok>*112$9.XkE+9F+8GU"
        if(ok:=FindText(0, 0, 4096, 4096, 0.05, 0.05, ok4on))
            {
                X:=ok.1.x, Y:=ok.1.y
                MouseClick, left, x, y
            }
...
scoreboard(forceout = 0)
{
    ...
    ;	}
    while (ook:=FindText(288, 482, 2859, 975, 0.01, 0.01, scoreboard) or forceout > 0)
    {
        if (ook:=FindText(288, 482, 2859, 975, 0.01, 0.01, scoreboard))
        {
   ...
   sleep 200
    send {enter}
        ...
        else ; both nonprime
        {
            ...
            goe()
            sleep 1500
            Text:="|<any_go>*114$14.rxdY289e2EUZV9U"
            loop.4
            {
            if (ok:=FindText(2499,87,2999,807, 0, 0, Text))
                    {
                        X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
                        Click, %X%, %Y%
                        sleep 20
...
newchangeaccs()
{
    ...
    loop
    {
        ...
        else
        {
            ...
            OutputDebug %p% `n
            Run steam.exe  -login %ha% %arguments% %p% -low ,%steam_folder%,,steam_pid
            ;run_command =steam.exe -inhibitbootstrap -single_core -login %ha% -applaunch 730 -silent -novid -nocache -nohltv -softparticlesdefaultoff -nopreload -hijack -limitvsconst  +mat_disable_fancy_blending -r_emulate_g -refresh 10 -nominidumps -safe -noforcemparms -nomsa -nojoy -noaafonts -nosound -threads 1 -noshader -safe  -d3d9ex -nocrashdialog -h 480 -w 640  %p%  -low
            Start_Time := A_TickCount
            ...
            while (wait_time < delay )
            {
                ...
                wait_time := A_TickCount - Start_Time
                WinGetTitle, Var, Connection
                    If InStr(var,"Connection Error")
                    {
                        WinGet, var_pid, PID, %var%
                        Runwait taskkill /PID %var_pid% /F /T ,,hide
                    }
                    WinGetTitle, Var, Steam
                    If InStr(var,"Steam - Warning")
                    {
                        WinGet, var_pid, PID, %var%
                        Runwait taskkill /PID %var_pid% /F /T ,,hide
                    }
    ...
    while (x = 0 and wait_time < 300000 ) ; eda matlab wait kro for
    {
        ...
        exe_update_check()
        Text:="|<>0x3F454E@0.92$33.zzzzzzzzzzzzzzzzzzzzzzzzzzzzz7w0zzsz01zz7ky7zswTszz73zXzsszwTz77zzzsszzzz77k3zssy0Tz77zXzsszwTz73zXzswTwTz7ky3zsz01zz7y0Tzzzzzzzzzzzzzzzzw"

 if (ok:=FindText(1981-150000, 1265-150000, 1981+150000, 1265+150000, 0, 0, Text))
 {
   X:=ok.1.x, Y:=ok.1.y, Comment:=ok.1.id
    Click, %X%, %Y%
    x = 0
    ...
    OutputDebug loop break %wait_time% `n
    odebug = loop break
        videosettingfix := tenaccopen_check()
    ;forcere = 1
    IniRead,my_mode,status.ini,SectionName,my_mode
    tenaccopen()
    ha := All_UserInfo[1]Clipboard
    ,ha := StrSplit(ha, ",")
    ...
    while (i <= total_accounts_open)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                IniRead,map_name,status.ini,SectionName,map_name
                WinGetTitle, Var, Connection
                    while InStr(Var,"Connection Error")
                    {
                        WinGet, var_pid, PID, %var%
                        Runwait taskkill /PID %var_pid% /F /T ,,hide
                        WinGetTitle, Var, Connection
                ...
                WinGetTitle, Var, Steam
                    ...
                    Loop
                    {
                        ...
                        else if InStr(var1,"Steam - Warning")
                            {
                                WinGet, var_pid, PID, %var1%
                                Runwait taskkill /PID %var_pid% /F /T ,,hide
                            }
                            else
                                break
...
check_rank()
{
    ...
    while (i <= total_accounts_open)
    {
        ...
        if (errorlevel = 3) ;errorlevel = 1 testing
        {
            ...
            cx := sx + 3
            cy := sy + 36
            TargetFileServer = /crop=(920, 238, 133, 111,0)
            Runwait, %A_ScriptDir%\IrfanView\i_view64.exe /capture=1 %TargetFileServer% /silent /convert=G:\My Drive\file share\val\extra\screenshot\%A_UserName%_%rand%_%A_TickCount%.png
            ;Sleep 2000
        }
