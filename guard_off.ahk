...
Loop
{
  Text:="|<next>*135$19.zzziEHX/wcZzBG3alTlMjqCE7jzzw"

if (ok:=FindText(1922, 1265, 50, 50, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  Click, %X%, %Y%
  runwait "C:\Program Files (x86)\Nemex\Mouse Recorder Pro 2\Mouse Recorder Pro.exe" "C:\Users\starboy\Documents\My Recorded Scripts\Untitled0.m2s" -count1 -autorun -end
  
  Loop
  {
   Text:="|<date>*100$17.000000zs1zw30Q60AA0MM0Ek0lU1X03606A0AM0Ek1VU330Q7zkDy1"

if (ok:=FindText(3560, 382, 50, 50, 0, 0, Text))
      {
        CoordMode, Mouse
        X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
        Click, %X%, %Y%
MouseMove, 20, -120, 10, R
        ...
        Text:="|<darkDate>*105$17.000000001zs3zw7zwC1sQ1ss1lk3XU770CC0QQ0ss1lk7XUS7zwDzkTy0000008"

if (ok:=FindText(3558, 441, 15, 15, 0, 0, Text))
      {
        CoordMode, Mouse
        X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
        ;Click, %X%, %Y%
  ...
  x = 0
  Text:="|<arrow>*110$20.0D007s03z00zk0Ty0Dzk7zy3zzkzzwTzzjzzzzzzzzzkDw03z00zk0Dw03z00zk0Dw03z00zk0Dw03z00zk8"

if (ok:=FindText(3407, 437, 150, 150, 0, 0, Text))
{
...
if (ok:=FindText(3558, 441, 15, 15, 0, 0, Text))
  ...
  Loop
  {
   Text:="|<mail>*106$11.00000000000000000y3yDywTkHU70C0Q4wSzszky000001"

if (ok:=FindText(3117, 422, 150, 15, 0, 0, Text))
      {
        CoordMode, Mouse
        X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
        Click, %X%, %Y%
  ...
  Sleep 3000
RunWait  "C:\Program Files (x86)\Nemex\Mouse Recorder Pro 2\Mouse Recorder Pro.exe" "C:\Users\starboy\Documents\My Recorded Scripts\Untitled1.m2s" -count1 -autorun -end
Loop
{
Text:="|<finish>*140$20.zzzv3SybLjhxvvX0yzLjdpvv3Szzzzzzzs"

if (ok:=FindText(2040, 1266, 150, 150, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  Click, %X%, %Y%
...
if (ok:=FindText(960, 960, 960, 960, 0, 0, Text))
  ...
  loop
  {
          Text:="|<popup>*135$36.zzzzzzy6z3VUxmytQjxywxDjtyxxVUxywxwjxmytSjy633VUzzzzzzzzzzzzU"

if (ok:=FindText(2196, 1416, 50, 50, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
   Click, %X%, %Y%
  ...
  Loop
  {
   Text:="|<manage>*130$43.zzzzzzznbStrkk9n7AlrdwlfWOnwy8qp99W1JEP44xDdfZWvSbolusSkkDzzzzzzw"


if (ok:=FindText(2097, 1165, 200, 40, 0, 0, Text))
      {
        CoordMode, Mouse
        X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
...
if (ok:=FindText(2196, 1416, 50, 50, 0, 0, Text))
{
  ...
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
   Click, %X%, %Y%
    ...
    Loop
  {
    Text:="|<turnoff>*90$23.zzzzzzzzzzzzzzzzzzzzzzzzzzzk0zzyTzzwzzztxwznvtzbrnzDjbyTTDwyyTtxwzntlzbn3zDkbzzzzzzzzzzzzzzzzzzz"

if (ok:=FindText(1485, 1215, 20, 20, 0, 0, Text))
      {
        CoordMode, Mouse
        X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
        ;x = x + 15
    ...
    Loop
  {
   Text:="|<turnoffConfirm>*160$8.zzzTzzzQqRbMrxzNr8"

if (ok:=FindText(1985, 1150, 15, 15, 0, 0, Text))
      {
        CoordMode, Mouse
        X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
        ;x = x + 15
 ...
 RunWait "C:\Program Files (x86)\Nemex\Mouse Recorder Pro 2\Mouse Recorder Pro.exe" "C:\Users\starboy\Documents\My Recorded Scripts\Untitled7.m2s" -count1 -autorun -end
  Sleep 2000
  
  RunWait "C:\Program Files (x86)\Nemex\Mouse Recorder Pro 2\Mouse Recorder Pro.exe" "C:\Users\starboy\Documents\My Recorded Scripts\Untitled3.m2s" -count1 -autorun -end
  

  
 ;RunWait "C:\Program Files (x86)\Nemex\Mouse Recorder Pro 2\Mouse Recorder Pro.exe" "C:\Users\starboy\Documents\My Recorded Scripts\Untitled2.m2s" -count1 -autorun -end

}

Text:="|<popup>*135$36.zzzzzzy6z3VUxmytQjxywxDjtyxxVUxywxwjxmytSjy633VUzzzzzzzzzzzzU"

if (ok:=FindText(2195, 1417, 150, 150, 0, 0, Text))
{
  CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  Click, %X%, %Y%
...
Text:="|<steam>*100$12.0000yTyTyTyTyTyTyTyTyTyTyTyTyTU"

if (ok:=FindText(1804, 794, 1500, 50, 0, 0, Text))
{
