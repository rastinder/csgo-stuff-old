...
IniRead, total_accounts_open, status.ini,SectionName , total_accounts_open
   global total_accounts_open
   store_it := total_accounts_open
    total_accounts_open = 1
    
    
    Loop
    {
      x = 0 
	Text:="|<voting>*129$13.S8A42/A5aGn8Nzzzzzzzzzzzzz00008"
			...
			loop, %store_it%
				{
					x1 := placing("c" , "x")
					,y1 := placing("c" , "y")
					Text:="|<dizzy>*99$17.zzz0zy38g01M5ck0/zzrzzy"
					if (ok:=FindText(x1, y1, "323", "248", "0.1", "0.1", Text))
						{
							X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
							Click, %X%, %Y%
                            OutputDebug dizzy %x% %y%
						}
					total_accounts_open++	
				}
			total_accounts_open = 1
			loop, %store_it%
				{
					x1 := placing("c" , "x")
					,y1 := placing("c" , "y")
					Text:="|<shoots>*68$21.0zzs3AY40020000144"
                    if (ok:=FindText(x1, y1, "323", "248", "0.1", "0.1", Text))
						{
							X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
							Click, %X%, %Y%
                            OutputDebug shoots %x% %y%
						}
                    Text:="|<shoots2>*75$19.zzzkzns00C986403zzzU"
					if (ok:=FindText(x1, y1, "323", "248", "0", "0", Text))
						{
							X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
							Click, %X%, %Y%
                            OutputDebug shoots2 %x% %y%
						}
					total_accounts_open++
				}	
			total_accounts_open = 1
			loop, %store_it%
				{
					x1 := placing("c" , "x")
					,y1 := placing("c" , "y")
					Text:="|<lake>*81$14.DnnYEl0AE042"
                    if (ok:=FindText(x1, y1, "323", "248", "0.1", "0.1", Text))
						{
							X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
							Click, %X%, %Y%
                            OutputDebug lake2 %x% %y%
						}
                    Text:="|<lake>*90$14.zzvvyk1g4M07zzU"
					if (ok:=FindText(x1, y1, "323", "248", "0.1", "0.1", Text))
						{
							X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
							Click, %X%, %Y%
                            OutputDebug lake %x% %y%
						}
					total_accounts_open++
				}	
		}
