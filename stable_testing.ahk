...
play_pr2()
  {
   ...
   run vote_map.exe
    Loop
          ...
          osd := total_pr2_done
          while ( ElapsedTime <= 2400000  and total_pr2_done <= total_accounts_open)
                    {
    ...
    loop
      {
        ...
        if (ErrorLevel = 1)
          ...
          { 
            Run	vac.exe -single_core -login %ha% -applaunch 730 -silent -windowed -novid  -nobootstrapupdate -nocache  -nohltv -nominidumps -nopreload -refresh 30 -noforcemparms -nomsaa -clearbeta -nojoy -noaafonts -nosound -threads 1 -noshader -nofbo -Heapsize 1048576 -d3d9ex -nocrashdialog -h 480 -w 640 -nodcaudio -16bpp %p% -low 
        sleep %delay%
            i++
            if (i = i_value)
              {
