...
SetMouseDelay, 300
!p::Pause
F4::OutputDebug %matchcount% done till ac num %set% , last_match=%time_since_last_match%
;F6::play_160()
F6::play_1515()
F7::changeaccs()
F8::tenaccopen()
...
Sleep 100
  ...
  if (time_since_last_match >= 2200000)
  {
                        ...
                        {    
                            OutputDebug normal sleep %rounddelay%
                             ...
                             while(ElapsedTime < rounddelay )
                                {
                                 OutputDebug Wait time %ElapsedTime%/%rounddelay%
                                  if (t = "no" and ElapsedTime > rounddelay * 0.80)
                                 ;if (t = "asd")
                                  {
                                  getteam()
                                  ;getscore()
...
ImageSearch, FoundX, FoundY, 0, 0, 3200, 960, *5 ok.png
  ...
  while (ok:=FindText(1280, 480, 1280, 480, 0, 0, Text))
    {
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  Click, %X%, %Y%
    }   
    ...
    ImageSearch, FoundX, FoundY, 0, 0, 3200, 515, *1 network_error.png
    ...
    Text2:="|<>*190$21.zzzwFssRyyrrjjzBZvxqr0+67zzzU"
      timer = 0
      ;while (ErrorLevel = 0)
    while (ok:=FindText(1700, 380, 1780, 580, 0.1, 0.1, Text) or ok:=FindText(1700, 380, 1780, 580, 0.1, 0.1, Text1) or ok:=FindText(1700, 380, 1780, 580, 0.1, 0.1, Text2))
      {
        OutputDebug network error detected
      timer++
      X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
      y := y + 15
      x := x + 40
      ...
      ImageSearch, FoundX, FoundY, 0, 0, 3200, 960, *20 network_error.png
...
if (ok:=FindText(2103, 1171, 700, 150, 0.3, 0.3, Text))
{ 
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
...
sleep %zz%	
sleep %zz%	
MouseClick, left,	3180,125
sleep %zz%	
MouseClick, left,3057‬,156
sleep %zz%	
MouseClick,left,3057‬,156,,90,,
sleep %zz%	
Text:="|<>*131$12.sTkDkDkDsTzvU3000003U"

if (ok:=FindText(3057, 156, 15, 15, 0, 0, Text))
{
  ;CoordMode, Mouse
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
   Click, %X%, %Y%
...
sleep %zz%	
MouseClick, left,	2910,292
sleep %zz%	
MouseClick, left,	2963,290
a5 := Clipboard
Clipboard := ""
;OutputDebug a4 %Clipboard%
...
sleep %zz%	
sleep %zz%
MouseClick, left,	2821,788 ; click dp
sleep %zz%	
sleep %zz%	
MouseClick, left,	2944,776
sleep %zz%	
Text:="|<>*129$71.zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzysC7s8BUbMTztrhrrrPT6rTzpjPbjiqyhiTzPSrj31g5fSzy2xiSyrPv6tztpvRxxqrrBrzrUC7vvZUjMTzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz"

if (ok:=FindText(2944, 776, 150, 150, 0, 0, Text))
{
...
sleep %zz%
sleep %zz%
MouseClick, left,	%a%, %b% ;msg
sleep %zz%
;MouseClick,left,‬,56,,90,,R
;sleep %zz%
;MouseClick,left,-45‬,83,,90,,R
;sleep %zz%
...
Text:="|<>*114$23.s0070007000C030Q060w1w1w0Q3z3k7z7UDyXyTzz0zwS1zU87y00Tw00zs01zk03z80zyk3zxsTzvnzzzzzs"
...
Text:="|<>*134$16.zzzsTzUzy3zsDzVzzzzkQy1XkET0XzzTzzy"
  while (ok:=FindText(1600, 480, 1600, 480, 0, 0, Text))
  {
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  Click, %X%, %Y%
...
Text:="|<>*121$26.zzzzzzzzzzzzzyyD3DDRirpjrxvPxzC2zTrCrPZraD3Dzzzzzzzzzzzzzzzzzzzzzzzzzs"
  while (ok:=FindText(1600, 480, 1600, 480, 0, 0, Text))
{
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
Click, %X%, %Y%
...
if (ok:=FindText(2103, 1171, 700, 150, 0.3, 0.3, Text1))
{ 
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  MouseClick, left,%X%, %Y%,1,50,,
  sleep 500
...
Sleep 100
                                  ...
                                  send {enter}
                                  sleep 2000
                                  IniWrite, %Clipboard% mg_cs_agency , C:\Program Files (x86)\Steam\steamapps\common\Counter-Strike Global Offensive\csgo\cfg\autoexec.cfg,echo ,
...
Text:="|<play_game>*144$27.zzzzy2zgtrLsrSuzLLkLmxyyy3jrrrRyy1xjzzzzw"
while (ok:=FindText(2103, 1171, 700, 150, 0.3, 0.3, Text))
{ 
  X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
  MouseClick, left,%X%, %Y%,1,50,,
  sleep 500
