...
t = no
                            StartTime := A_TickCount
                            ElapsedTime := A_TickCount - StartTime
OutputDebug join_msg 
  Text:="|<join_msg>142@0.88$39.kAAAlaM0EEE08E2220161UEK4/U"
while(!ok:=FindText(2654, 912, 150, 10, 0, 0, Text))
  {
    Sleep 100
                  Text1:="|<new_settings>*195$10.DkS0k03Vy7zy"
              if (ok:=FindText(2582, 777, 16, 100, 0, 0, Text1))
                            return  
                             if (ElapsedTime > 4000) 
                            {
								OutputDebug 4000
...
OutputDebug join_msg 
  Text:="|<join_msg>142@0.88$39.kAAAlaM0EEE08E2220161UEK4/U"
while(!ok:=FindText(2654, 432, 150, 10, 0, 0, Text))
  {
      Sleep 100
                    Text1:="|<new_settings>*195$10.DkS0k03Vy7zy"
              if (ok:=FindText(2582, 297, 16, 100, 0, 0, Text1))
                            return  
                             if (ElapsedTime > 4000) 
                            {
								OutputDebug 4000
  ...
  OutputDebug msg found
						...
						if ( my_mode = 15)
                        wins++
              Text:="|<new_settings>*195$10.DkS0k03Vy7zy"
              if (ok:=FindText(2582, 297, 16, 100, 0, 0, Text))
                            return                        
}}
topbat()
    {   
...
dis_8_acc(which)
    {
		...
		if instr(which, "all") or if instr(which, "bottom") 
		{
         MouseClick, left,450,545,,0,,
      Click down
      sleep 100
      ...
      sleep 100
		...
		if instr(which, "all") or if instr(which, "top") 
		{
             MouseClick,left,550,180,,10,,
        Click down
        sleep 100
			...
			sx := sx%a%
				 ...
				 Loop
				{
					if ssscore = -1
						break
					text := text_%ssscore%
					 if (ok:=FindText(sx, sy, tx, ty, "0", "0", Text))
					{
												cts := ssscore
						cts += 0
                       ,getct := cts
