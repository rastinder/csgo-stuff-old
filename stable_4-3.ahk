...
variables()
{
    ...
    else
    {
        ...
        Text:="|<new_settings>*99$14.U0M067VlswSC7VU0M07E/y7zVzzzU"
...
play()
{
    odebug = ran mode %my_mode%, swap %swap_accs_disable%, till %matchlimit%,ro %rank_open%, `n mwin %more_wins% ,mpr %medal%
    SLEEP 2000
    changeaccs()
    loop
    {
        ...
        if ( loading = 1 )
        {
            ...
            theaccept = 0
            If (ErrorLevel = 0 and theaccept < 8)
            {
                theaccept++
                sleep 1000
                ImageSearch, FoundX, FoundY, 0, 0, 3200, 540, *2 accept.png
            ...
            else
            {
                ...
                if (my_mode = 15)
                {
                    ...
                    ElapsedTime_ms := A_TickCount - StartTime_ms
                    reconnect()
                    ElapsedTime_ms3 := A_TickCount - ElapsedTime_ms
                    dis_8_acc(t)
                    super_dis(t)
                    ElapsedTime_ms2 := A_TickCount - ElapsedTime_ms3
                }
                if (my_mode = 16)
                {
                ...
                IniRead,my_mode,status.ini,SectionName,my_mode
                matchcount++
                cm := Convert.TickCount.toMinutes(ElapsedTime_ms)
                ,cs := Convert.TickCount.toSeconds(ElapsedTime_ms)
                ,cm2 := Convert.TickCount.toMinutes(ElapsedTime_ms2)
                , cs2 := Convert.TickCount.toSeconds(ElapsedTime_ms2)
                odebug = match:%matchcount% t:%cm%:%cs%_%cm2%:%cs2%
                loading := "0"
                ,do1614 := "0"
                ,cts := "0"
                ,tss := "0"
...
super_dis(which)
{
    ...
    ,new_settings:="|<new_settings>*99$14.U0M067VlswSC7VU0M07E/y7zVzzzU"
...
dis_8_acc(which)
{
    loop, 2
    {
        while (ok !=0)
        {
            Text:="|<in_match_check>0xFA2E2A@0.71$6.zznkyT3nzzU"
            if instr(which, "all")
                ok:=FindText(0, 0, 2660, 960, 0.15, 0.15, Text)
            else if instr(which, "up")
                ok:=FindText(0, 0, 2660, 460, 0.15, 0.15, Text) 
            else if instr(which, "down")
                ok:=FindText(0, 480, 2660, 960, 0.15, 0.15, Text)
            for i,v in ok
            {
...
{ 
    ...
    loop, 2
    {
        while (ok !=0)
        {
            ...
            if (lobby = 0)
                ok:=FindText(366, 10, 3120, 560, 0.15, 0.15, reconnect_r)
            else if(lobby != "down")
...
{ 
    ...
    while (x = 0)
    {
        ...
        if changeaccs()
            return
        ;TText:="|<new_settings>*99$14.U0M067VlswSC7VU0M07E/y7zVzzzU"  ; are we on home screen?
        Text:="|<new_settings>*99$14.U0M067VlswSC7VU0M07E/y7zVzzzU"
        if (ok:=FindText(2582-16, 777-100, 2582+16, 777+100, 0, 0, Text))
        {
            X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
    ...
    if (ok:=FindText(3055-15, 157-15, 3055+15, 157+15, 0, 0, Text) && lobbycount >= 1 && lobbycount <= 3 && swap_accs = 0)
    {
        lobbycount++
    ...
    if (lobbyfull = 1)
    {
        ...
        if (lobbystatus2 = 1 and lobbystatus1 = 1 and medal != 1)
        {
            ...
            else if (ok:=FindText(0, 0, 3200, 960, 0.1, 0.1, unranked))
            {
                IniRead,rank_variation_old,status.ini,SectionName,rank_variation
                Iniwrite,%rank_variation_old%,status.ini,SectionName,rank_variation_old
                matchcount := matchlimit - 2
...
single_inv()
{
    ...
    Text:="|<notinlobby>*108$8.ks4M63XswT7XtyTbzyTbU"
...
check_rank()
{
    ...
    ,countp := "0"
    ,errorlevel := "1"
    ,Text:="|<new_settings>*99$14.U0M067VlswSC7VU0M07E/y7zVzzzU" ; are we home screen
    if (ok:=FindText(0, 0, 3200, 1920, 0.1, 0.1, Text))
    {
        for j,v in ok
        {
    ...
    while (i <= total_accounts_open)
    {
        ...
        if (errorlevel = 1) ;errorlevel = 1 testing
        {
            countr++
