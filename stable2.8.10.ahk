...
NumpadAdd::Pause
NumpadSub::print("imp")
F4::OutputDebug %matchcount% done till ac num %set% `n  pr2 = %total_pr2_done% in %MyTicks%min
F6::play_160()
;F6::play_1515()
F7::changeaccs()
F8::tenaccopen()
    ...
    If InStr(get,"GO") or InStr(get,"G")
      {
          ...
          BlockInput, On          ; BlockInput, off
           Sleep 10
           MouseClick, left,3058,473,,10,,  ; uper wala go
           Sleep 10
    ...
    If InStr(get,"GO") or InStr(get,"G")
      {
        ...
        BlockInput, On          ; BlockInput, off
        Sleep 10
        MouseClick, left,3058,956,,10,,     ; niche wala go
        Sleep 10
        ...
        if (ok:=FindText(1280, 480, 1280, 480, 0, 0, Text) or play_now = 1)
            {
                ...
                Run	c:\Program Files (x86)\Steam\Steam1.exe -nobootstrapupdate -single_core -login %ha% -applaunch 730 -silent -novid -nocache -nohltv -nominidumps -safe -noforcemparms -nomsa -nojoy -noaafonts -nosound -threads 1 -noshader  -d3d9ex -nocrashdialog -h 480 -w 640  -low
                Sleep 5000
                time_since_last_match := ""
                x = 0
                timer = 0
...
newchangeaccs()
  {
   ...
   ,pos20 := "-x 2563 -y 1456"
    ...
    loop
      {
        ...
        if (ErrorLevel = 1)
          ...
          { 
            ;Run	c:\Program Files (x86)\Steam\Steam.exe
            Run	C:\Program Files (x86)\Steam\steam.exe -nobootstrapupdate -single_core -login %ha% -applaunch 730 -silent -novid -nocache -nohltv -nominidumps -safe -noforcemparms -nomsa -nojoy -noaafonts -nosound -threads 1 -noshader  -d3d9ex -nocrashdialog -h 480 -w 640 %p% -low 
            Start_Time := A_TickCount
    ...
    Start_Time := A_TickCount
    wait_time := A_TickCount - Start_Time
    while (x = 0 && wait_time < 60000 ) ; earlier it was 600000 pta ni ki soch pai a
            {
                wait_time := A_TickCount - Start_Time
                error()
                ControlClick , OK, VAC bypass
        ...
        else
            {
              ...
              if (errorlevel = 0)
            ...
            else
                {
                  ...
                  p := pos%i%
                  ; Run	c:\Program Files (x86)\Steam\Steam.exe
                  Run	C:\Program Files (x86)\Steam\steam.exe -nobootstrapupdate -single_core -login %users% -applaunch 730 -silent -novid -nocache -nohltv -nominidumps -safe -noforcemparms -nomsa -nojoy -noaafonts -nosound -threads 1 -noshader  -d3d9ex -nocrashdialog -h 480 -w 640  %p% -low 
                  sleep  10000
                  timer = 0
                  ImageSearch, FoundX, FoundY, %sx%, %sy%, %tx%, %ty%, *50 tv.png     
                  while (errorlevel != 0)
