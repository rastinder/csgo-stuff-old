...
Gui, Add, Text, vMyText21 cYellow, %A_UserName%,  ;XXxxxxxxxxxxxxxxxxxxxxXXX YYyyyyyyyyyyyyyyyyyyyyYYY  ; XX & YY serve to auto-size the window.
...
F4::print("status","status","0","lan") ; folder,comment,repeat,location
F6::play_160()
;F6::play_1515()
F7::changeaccs()
F8::tenaccopen()
...
if (odebug = tText[1]  )
	return
    jjj = %matchcount% done till ac num %set%
tText[20] := jjj
,tText[19] := tText[18]
,tText[18] := tText[17]
,tText[17] := tText[16]
  ...
  {   
    ...
    loop
            {
              ...
              if (ok:=FindText(0, 0, 3200, 960, 0, 0, Text))
				  ...
				  if (onetime = 5000)
				{
					print("logs","wrong detected scoreboard","1","lan") ; folder,comment,repeat,location
				}
            }
    error()
    x = 0 
    ...
    if (matchcount = 0)
    ...
    else
      {
        ...
        if (ok:=FindText(3055-15, 157-15, 3055+15, 157+15, 0, 0, Text) && matchcount != 0 && swap_accs = 0) 
          {
            odebug = normal invite initiated
    ...
    if (lobbystatus2 != lobbystatus1)
      {
        odebug = one is prime other is non prime 
        odebug = %matchcount% done till ac num %set% , mode %my_mode%
        IniWrite, %lobbystatus1%,%lobbystatus2% , status.ini,prime_status, prime_%set%
		print("swap_needed",A_UserName,"1","lan") ; folder,comment,repeat,location
        matchcount = matchlimit++
        changeaccs()
      }
...
update_client_check(dhkka)
{
	...
	{
		MouseMove x,y
		odebug = update client check %play_now%
		Sleep 100
		play_now = 0
		...
		while(x = 0)
		{
			...
			loop
			{
               ...
               ; ControlSend,,{Esc},csgo.exe
				Text:="|<close>*115$56.zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzlyAlzzzzzsH20TzzzzyImYTzzzzzbAg1zzzzztH+lzzzzzy40U7zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzs"
				if (ok:=FindText(0, 0, 3200, 1920, 0.11, 0.11, Text1))
				{ 
					X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
					MouseClick, left,%X%, %Y%,1,50,,
					sleep 500
...
;      odebug  ------------
	...
	getteam()
	{
		...
		{
			if (ok:=FindText(0, 885-60, 3200, 885+60, 0, 0, Text))
			{
				t = down
				odebug = t.gun t = %t%
				get := ""
...
;						}
	...
	home()
	{
      ...
      ;Text:="|<has medal>*154$26.zzzzzzzzzzxzzzzDzzzVzzzkDzzw3zzzVzzzsTzzzzzzznzzzzzzzzzzzzzzy"
		if (ok:=FindText(0, 0, 3560, 1160, 0, 0, Text))
		{  
			X:=ok.1.1, Y:=ok.1.2, W:=ok.1.3, H:=ok.1.4, Comment:=ok.1.5, X+=W//2, Y+=H//2
			Click, %X%, %Y%
        ...
        ,ty := ty%b%
			...
			else
			{
				...
				else
				{
					...
					p := pos%i%
					IniRead,arguments,status.ini,SectionName,arguments
					Run,steam.exe %arguments% -login %users% %p%  -low  ,%steam_folder%,,steam_pid
...
check_rank()
	{
		RunWait, net use \\192.168.178.11 '  /user:Administrator,,Hide
		odebug = CHKING ranks
		sx1 := "605"
      ,sy1 := "91"
      ,tx1 := "640"
	  ...
	  ,errorlevel := "1"
        ...
        ,tx := tx%a%
        ,ty := ty%b%
		Loop, Files, \\192.168.178.11\c\DLL\rank\*.png
			{
			;OutputDebug `n %A_LoopFileLongPath%
			;OutputDebug `n %A_LoopFileFullPath%
			;OutputDebug `n %A_LoopFileName%
			;oDebug = %A_LoopFileLongPath%
			;oDebug = \\192.168.178.11\c\DLL\5465_8969187.png
			ImageSearch, FoundX, FoundY, %sx%, %sy%, %tx%, %ty%, *50 \\192.168.178.11\c\DLL\rank\%A_LoopFileName%
			;ImageSearch, FoundX, FoundY, %sx%, %sy%, %tx%, %ty%, *10 \\192.168.178.11\c\DLL\%A_LoopFileName%
			;OutputDebug `n %errorlevel%
			;sleep 10
			...
			if (errorlevel = 0)
			{
				OutputDebug `n posi = %i% rank %A_LoopFileName%
				break
			}
			}
			;ok := FindText_GetTextFromScreen(sx, sy, tx, ty)
			;OutputDebug %ok%
			...
			if (errorlevel != 0)
			{
				...
				cx := sx + 1
				cy := sy + 1
				;OutputDebug, `n %sx% %sy%, %cx% %cy%
				TargetFileServer = /crop=(%cx%, %cy%, 33, 11,0)
				Run, %A_ScriptDir%\IrfanView\i_view64.exe /capture=1 %TargetFileServer%  /convert=\\192.168.178.11\c\DLL\rank\%A_UserName%_%rand%_%A_TickCount%.png
				Sleep 2000
			}
			i++
			,a++
		}
		odebug = %count% new ranks found
	  ...
	  ,errorlevel := "1"
        ...
        ,tx := tx%a%
        ,ty := ty%b%
		Loop, Files, \\192.168.178.11\c\DLL\pr\*.png
			{
			;OutputDebug `n %A_LoopFileLongPath%
			;OutputDebug `n %A_LoopFileFullPath%
			;OutputDebug `n %A_LoopFileName%
			;oDebug = %A_LoopFileLongPath%
			;oDebug = \\192.168.178.11\c\DLL\5465_8969187.png
			ImageSearch, FoundX, FoundY, %sx%, %sy%, %tx%, %ty%, *10 \\192.168.178.11\c\DLL\pr\%A_LoopFileName%
			;ImageSearch, FoundX, FoundY, %sx%, %sy%, %tx%, %ty%, *10 \\192.168.178.11\c\DLL\%A_LoopFileName%
			;OutputDebug `n %errorlevel%
			;sleep 10
			;ImageSearch, FoundX, FoundY, %sx%, %sy%, %tx%, %ty%, *3 crash.png 
			...
			if (errorlevel != 0)
			{
				...
				Run, %A_ScriptDir%\IrfanView\i_view64.exe /capture=1 %TargetFileServer%  /convert=\\192.168.178.11\c\DLL\pr\%A_UserName%_%rand%_%A_TickCount%.png
				Sleep  2000
			}
			i++
			,a++
		}
		odebug = %count% new pr found
	...
	print(folder,comment,repeat,location) ; folder,comment,repeat,location
	{
		...
		if(print = 1)
		{
			if (repeat = 1)
				repeat = %A_TickCount%
			if (location == "lan")
				location = \\192.168.178.11\c\DLL\
			else
				location = %A_ScriptDir%
			Run, %A_ScriptDir%\IrfanView\i_view64.exe /capture=1 /convert=%location%\%folder%\%comment%_%set%_%matchcount%_%repeat%_%A_UserName%.png
		}
		else
		{
